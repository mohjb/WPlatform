...
/*app.get('/list/:tbl/:lm', function (req, res) {
		const str='express:app.get:list:';
		let p=req.params
		...
		if(!ss|| !ss.usr|| !ss.usr.state|| !ss.usr.state.active )
			return res.json({error:'invalid credintials'});
		let uid=ss.usr.id,admin=ss.usr.state.admin
		ss.expire=now+app.sessions.timeoutPeriod;
		pool.getConnection().then(conn =>{
			function f(p,retFunc){
				let tbl=app.Tables.find(i=>i[0]==p.tbl),lm=p.lm
				if(! tbl)
					return res.json({error:'invalid :'+p.tbl});
				function trns(){
					let tbl=app.Tables.Acc
					return m=conn.query("SELECT `id` from `'+tbl[0]+'` where `uid`=?",[uid])
						.then(accs=>{let s='('+accs.join(',')+')',tbl=app.Tables.trans
			...
			if(!p.a)
				return f(p,res.json)
			let a=[];
			p.a.forEach(//TODO: fix conn/incomplete-Promises race-problem
				e=>
					f((e,i),
						r=>(
							a.push((e.data=r,e)),
